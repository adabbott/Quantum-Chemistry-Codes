...
for iteration in range(1, CEPA0_MAX_ITER+1):
    #read off terms from CEPA diagrammatic expression for t amplitudes and compute the energy
    t_mixed = np.einsum('cjkb, ikac -> ijab', I_mo[V,O,O,V], t_amp)
    t_amp_new = (1/e_ijab)*(I_mo[O, O, V, V]  + (1/2)*np.einsum('cdab, ijcd -> ijab', I_mo[V,V,V,V], t_amp)  + (1/2)*np.einsum('ijkl, klab -> ijab', I_mo[O,O,O,O], t_amp)  + t_mixed - t_mixed.transpose((1,0,2,3)) - t_mixed.transpose((0,1,3,2)) + t_mixed.transpose((1,0,3,2)))
    #t_amp_new = (1/e_ijab)*(I_mo[O, O, V, V]  + (1/2)*np.einsum('cdab, ijcd -> ijab', I_mo[V,V,V,V], t_amp) + (1/2)*np.einsum('ijkl, klab -> ijab', I_mo[O,O,O,O], t_amp) + np.einsum('cjkb, ikac -> ijab', I_mo[V,O,O,V], t_amp) - np.einsum('cikb, jkac -> ijab', I_mo[V,O,O,V], t_amp) - np.einsum('cjka, ikbc -> ijab', I_mo[V,O,O,V], t_amp) + np.einsum('cika, jkbc -> ijab', I_mo[V,O,O,V], t_amp)) 
