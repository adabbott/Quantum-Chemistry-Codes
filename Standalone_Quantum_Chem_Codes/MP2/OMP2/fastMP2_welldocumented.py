...
DM2cor = np.zeros((ntotal,ntotal))

for i in range(OMP2_MAX_ITER):
    h1 = np.einsum('pq, qr -> pr', H, C_block)
    fock = h1 + np.einsum('piqi->pq', I_mo[:,O,:,O])
    fprime = np.copy(fock) #fprime is the off diagonal part of fock operator
    np.fill.diagonal(fprime,0)
    
    term2 = np.einsum('ac,cbij->ijab',fprime, tamp) 
    term3 = np.einsum('ki,abkj->ijab',fprime, tamp)  
    tamp = I_mo_MP2 + (1/2)*(term2 - term2.transpose(0,1,3,2))                         \
                    - (1/2)*(term3 - term3.transpose(1,0,2,3))
    tamp /=   orb_energies[O,x,x,x] + orb_energies[x,O,x,x]                      \
            - orb_energies[x,x,V,x] - orb_energies[x,x,x,V]
    #build virtual and occupied space of our 1 and 2 e- density matrices
    DM1cor[V,V] =  (1/2)*np.einsum('ijac,bcij->ba',tamp,tamp)#these might be wrong
    DM1cor[O,O] = -(1/2)*np.einsum('jkab,abik->ji',tamp,tamp)
    DM2cor[O,O,V,V] = tamp
    ...
    X[V,O] = (F - F.T)[V,O]/(orb_energies[O,x]-orb_energies[x,V])
