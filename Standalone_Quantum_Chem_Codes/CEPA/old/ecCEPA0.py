...
t_virt = np.zeros((ntotal-nocc,ntotal-nocc))
t_amp = np.zeros((nocc,nocc,ntotal-nocc,ntotal-nocc))
Energy = 0.0
for iteration in range(1, CEPA0_MAX_ITER+1):
    #form t_virt using density fitting
    for c in range(ntotal-nocc):
        for d in range(ntotal-nocc):
            #for a particular set of virtual indices c and d take the "matrix slice" out of that 3d matrix 
            virt_relevant_2_e_integrals = np.einsum('aR, bR -> ab', Bab[c,:,:], Bab[d,:,:])
            for a in range(ntotal-nocc): #iterate over all virtuals
                for b in range(ntotal-nocc): 
                    t_virt[a,b] = (1/2)*(virt_relevant_2_e_integrals[a, b] - virt_relevant_2_e_integrals[b, a])
    #read off terms from CEPA diagrammatic expression for t amplitudes
    t_amp_new = (1/e_ijab)*(I_mo1 + np.einsum('ab, ijcd ->ijab', t_virt, t_amp) + (1/2)*np.einsum('ijkl, klab -> ijab', I_mo3, t_amp) + np.einsum('cjkb, ikac -> ijab', I_mo4, t_amp) - np.einsum('cikb, jkac -> ijab', I_mo4, t_amp) - np.einsum('cjka, ikbc -> ijab', I_mo4, t_amp) +np.einsum('cika, jkbc -> ijab', I_mo4, t_amp))
