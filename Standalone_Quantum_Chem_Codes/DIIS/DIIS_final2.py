    ...
    if int(config['SCF']['diis']) == 1 and int(config['SCF']['diis_start']) <= iteration:  
       #do DIIS
        ...
        if n >= 2:
            ...
            f = np.zeros(np.size(Pa, 0)) #size of Pa,Pb along axis 0
            f = np.append(f, -1)
            #add a row/column of -1's and final element 0
            add_row    = np.zeros(np.size(Pa, 1)) + -1
            add_col = np.zeros(np.size(Pa, 1)) + -1
            add_col = np.append(add_col, 0)
            Pa = np.insert(Pa, np.size(Pa, 0), add_row, axis=0)
            Pa = np.insert(Pa, np.size(Pa, 1), add_col, axis=1)
            Pb = np.insert(Pb, np.size(Pb, 0), add_row, axis=0)
            Pb = np.insert(Pb, np.size(Pb, 1), add_col, axis=1)
            #P matrices are now built, and we have our f's, solve for q's
            P = (Pa + Pb)/2  #average Pa and Pb to fix singular matrix issues
