...
for iteration in range(1, SCF_MAX_ITER+1):
    ...
    Ja = np.einsum('pqrs,rs->pq', I, D) #total repulsion of alpha e-
    Jb = np.einsum('pqrs,rs->pq', I, D) #total repulsion of beta e-
    Ka = np.einsum('prqs,rs->pq', I, D) #total exchange of alpha e-
    Kb = np.einsum('prqs,rs->pq', I, D) #total exchange of beta e-
    Fa = H + Ja - Ka + Jb
    Fb = H + Jb - Kb + Ja
