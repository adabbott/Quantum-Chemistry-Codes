...
for iteration in range(1, SCF_MAX_ITER+1):
    ...
    #do DIIS
        ...
        if n >= 2:
            ...
            P = np.bmat([[pa, zero], [zero, pb]]) 
            
            add_row    = np.zeros(np.size(P, 1)) + -1
            add_col = np.zeros(np.size(P, 1)) + -1
            add_col = np.append(add_col, 0)
            P = np.insert(P, np.size(P, 0), add_row, axis=0)
            P = np.insert(P, np.size(P, 1), add_col , axis=1)
            print(P) 
            f = np.zeros(np.size(P, 0)) #size of P along axis 0
            f[-1] = -1
            #P matrices are now built, and we have our f, solve for q
            q = np.linalg.solve(P, f)
            #optimized F's
            Fa = sum(q[i]*a_focks[i] for i in range(n))
            Fb = sum(q[i+n]*b_focks[i] for i in range(n))
