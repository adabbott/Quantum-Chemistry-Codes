...
for iteration in range(1, SCF_MAX_ITER+1):
    ...
    Ca = C_a[:, :nalpha]
    Cb = C_b[:, :nbeta]
    Da = np.einsum('ip,qi->qp', Ca.T, Ca)
    Db = np.einsum('ip,qi->qp', Cb.T, Cb)
    #Da = np.einsum('pi,iq->pq', Ca, Ca.T)
    #Db = np.einsum('pi,iq->pq', Cb, Cb.T)
    #Da = np.einsum('pi,qi->pq', Ca, Ca)
    #Db = np.einsum('pi,qi->pq', Cb, Cb)
    #Da = np.einsum('ip,iq->qp', Ca.T, Ca.T)
    #Db = np.einsum('ip,iq->qp', Cb.T, Cb.T)

print('Final UHF Energy is: ', E_SCF)
...
#Dtotal = np.einsum('pi,qi->pq', Ca, Cb)
#Density = S.dot(Dtotal).dot(S)
#print(np.trace(Density))


D_block = np.einsum('pi,qi->pq', C_block, C_block)

def spinblockS(S):
    return np.kron(np.eye(2), S)

def transformS(S, C): 
    return np.einsum('Qp,pP->PQ',
           np.einsum('pq,qQ->Qp', S, C), C)  

#St = transformS(spinblockS(S), C_block)
St = spinblockS(S)
#print(St.dot(D_block).dot(St))
...
t1 = time.time()
print('UMP2 Correlation Energy is: ', fast_EMP2)
print('Final UMP2 Energy is: ', fast_EMP2+E_SCF)
print('UMP2 took %7.5f seconds' % ((t1-t0)))
...
def transformoei(hao, C):       #C has dimensions AO x MO, to convert each axis of h, g to MO basis by dotting it with 
    return np.einsum('mq,mp->pq',
           np.einsum('mv,vq->mq', hao, C), C)  
...
print('Starting Orbital Optimized UMP2...')
tx = time.time()
...
for i in range(OMP2_MAX_ITER):    
    ...
    print('U-OMP2 iteration%3d: Energy %20.12f  dE %1.5E' % (i+1, EOMP2, EOMP2 - EOMP2_old))
    if (abs(EOMP2 - EOMP2_old) < 1.e-10):
        break
    EOMP2_old = EOMP2
ty = time.time() 
print('Final U-OMP2 Energy is %10.12f' % (EOMP2))
print('Orbital Optimized UMP2 took %7.5f seconds' % ((ty-tx)))
