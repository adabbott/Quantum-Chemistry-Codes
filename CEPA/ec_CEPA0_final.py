...
for iteration in range(1, CEPA0_MAX_ITER+1):
    ...
    if int(config['CEPA0']['df']) == 0: 
        t_mixed = np.einsum('cjkb, ikac -> ijab', I_mo4, t_amp)
        t_amp_new = (1/e_ijab)*(I_mo1 + (1/2)*np.einsum('abcd, cdij -> ijab', I_movv, t_amp) + (1/2)*np.einsum('klij, abkl -> ijab', I_mo3, t_amp) + t_mixed - t_mixed.transpose((1, 0, 2, 3)) - t_mixed.transpose((0,1,3,2)) + t_mixed.transpose((1,0,3,2)))
        E_CEPA0 = (1/4)*np.einsum('ijab, ijab ->', I_mo1, t_amp_new)
