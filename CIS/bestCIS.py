...
import time
"""This CIS is the best. We only transform integrals we need, we optimize both our densitys and energies in SCF,
we sort our ci eigenvalues and eigenvectors and use the more reliable np.linalg.eig function to get them."""
config = configparser.ConfigParser()
config.read('Options2.ini') #pointing to our options.ini file for info on molecule
...
#we parse our C_blocks in these einsums to get only certain relevant terms
...
#do CIS
ta = time.time()
I_mo_cis = np.einsum('pQRS, pP -> PQRS', 
       np.einsum('pqRS, qQ -> pQRS', 
       np.einsum('pqrS, rR -> pqRS', 
       np.einsum('pqrs, sS -> pqrS', I_phys, C_block[:,V]), C_block[:,O]), C_block[:,O]), C_block[:,V]) 

HCIS = np.zeros((nvirt*nocc,nvirt*nocc))
strings = [] #collect strings of excitations, like 0 -> 2, for pretty printing later
str2 = '->' #one piece of our strings
w = -1 #a counter to build our matrix elements 
for i in range(nocc):
    for a in range(nvirt):
        w = w+1 #more counters to build matrix elements
        v = -1
        str1 = str(i) #other pieces of our strings
        str3 = str(a)
        strings.append(''.join((str1, str2, str3)))
        for j in range(nocc):
            for b in range(nvirt):
                v = v+1 #counter for matrix elements
                HCIS[w,v] = (orb_energies[a+nocc] - orb_energies[i]) * (i==j) * (a==b) + I_mo_cis[a, j, i, b] #CIS matrix element eqn
ECIS, CCIS = np.linalg.eig(HCIS) #diagonalize H, get excitation energies and CI coefficients
#stupid eig doesnt sort energies/vectors in increasing order, so we sort
CCIS = CCIS[:,ECIS.argsort()]
ECIS = np.sort(ECIS) #np.linalg.eig is stupid and doesnt sort eigenvalues/eigenvectors, so we do it here
tb = time.time()
print('CIS:')
for state in range(nvirt*nocc):
    ...
    print()
print('CIS took %7.5f seconds' % ((tb-ta)))
