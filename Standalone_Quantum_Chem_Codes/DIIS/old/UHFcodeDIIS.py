...
for iteration in range(1, SCF_MAX_ITER+1):
    ...
    #do DIIS
        ...
        a_errs.append(err_a)
        b_errs.append(err_b) 
        Pa = np.zeros((len(a_focks), len(a_focks))) 
        Pb = np.zeros((len(b_focks), len(b_focks)))
        if len(a_focks) >= 2:
            #build P matrices
            ...
            qa = np.linalg.solve(Pa, fa)
            qb = np.linalg.solve(Pb, fb)
            #delete last element of q to match sizes
            #optimized F's
            Fa = sum(qa[i]*a_focks[i] for i in range(len(a_focks)))
            Fb = sum(qb[i]*b_focks[i] for i in range(len(b_focks)))
            #need something involving nvector?
