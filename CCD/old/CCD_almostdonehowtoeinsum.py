...
"""set variable t0 = time.time()

later set variable t1 = time.time()

print('X took {:7.5f} seconds'.format((t1-t0))) 
or
print('X took %7.5f seconds' % ((t1-t0)))
"""
...
#for CCD terms
I_mo5 = np.einsum('pQRS, pP -> PQRS', 
       np.einsum('pqRS, qQ -> pQRS', 
       np.einsum('pqrS, rR -> pqRS', 
       np.einsum('pqrs, sS -> pqrS', I_phys, C_block[:,V]), C_block[:,V]), C_block[:,O]), C_block[:,O]) 
#all virtual terms, originally separated out for density fitted CEPA0
I_movv = np.einsum('pQRS, pP -> PQRS', 
       np.einsum('pqRS, qQ -> pQRS', 
       np.einsum('pqrS, rR -> pqRS', 
...
#t_amp_df = np.zeros((nocc, nocc, ntotal-nocc, ntotal-nocc))
Energy = 0.0
#Energy_df = 0.0

'''if int(config['CEPA0']['df']) == 1:
    #do dfCEPA0 
    t0df = time.time()   
    for iteration in range(1, CEPA0_MAX_ITER+1):
        for a in range(ntotal-nocc): #iterate over all virtuals
            for b in range(ntotal-nocc): 
                ...
                t_virt[:,:,a,b] = (1/2)*np.einsum('cd, ijcd -> ij', (I_cd - I_cd.T), t_amp_df)

        #read off terms from CEPA diagrammatic expression for t amplitudes
        t_mixed_df = np.einsum('cjkb, ikac -> ijab', I_mo4, t_amp_df)
        t_amp_new_df = (1/e_ijab)*(I_mo1 + t_virt + (1/2)*np.einsum('ijkl, klab -> ijab', I_mo3, t_amp_df) + t_mixed_df - t_mixed_df.transpose((1, 0, 2, 3)) - t_mixed_df.transpose((0,1,3,2)) + t_mixed_df.transpose((1,0,3,2)))
        E_CEPA0_df = (1/4)*np.einsum('ijab, ijab ->', I_mo1, t_amp_new_df)
    
        print('dfCEPA0 iteration %3d: energy %20.14f  dE %1.5E |dT| %1.5E ' % (iteration, E_CEPA0_df, E_CEPA0_df - Energy_df, np.linalg.norm(t_amp_new_df-t_amp_df)))
        if  (abs(Energy_df - E_CEPA0_df)) < 1.e-10 and (abs(np.linalg.norm(t_amp_new_df-t_amp_df)) < 1.e-10): 
            break
        t_amp_df = t_amp_new_df
        Energy_df = E_CEPA0_df
    t1df = time.time()
   # ta = time.time()
    #do regular CEPA0
    for iteration in range(1, CEPA0_MAX_ITER+1):
        t_mixed = np.einsum('cjkb, ikac -> ijab', I_mo4, t_amp)
        t_amp_new = (1/e_ijab)*(I_mo1 + (1/2)*np.einsum('abcd, ijcd -> ijab', I_movv, t_amp) + (1/2)*np.einsum('klij, klab -> ijab', I_mo3, t_amp) + t_mixed - t_mixed.transpose((1, 0, 2, 3)) - t_mixed.transpose((0,1,3,2)) + t_mixed.transpose((1,0,3,2)))
    ...
    print(E_SCF + E_CEPA0)
    print('Partially density fitted CEPA0 correlation energy:')
    print(E_CEPA0_df)
    print('Density fitting error:')
    print(E_CEPA0 - E_CEPA0_df) 
    print('df-CEPA0 took %7.5f seconds' % ((t1df-t0df)))
    print('CEPA0 took %7.5f seconds' % ((tb-t1df)))'''
 
t0 = time.time()
for iteration in range(1, CEPA0_MAX_ITER+1):
    t_mixed = np.einsum('cjkb, ikac -> ijab', I_mo4, t_amp)
    t_ccd1 = np.einsum('cdkl, klab, ijcd -> ijab', I_mo5, t_amp)
    t_ccd2 = np.einsum('cdkl, ikac, ljdb -> ijab', I_mo5, t_amp)
    t_ccd3 = np.einsum('cdkl, klca, ijdb -> ijab', I_mo5, t_amp)
    t_ccd4 = np.einsum('cdkl, kicd, ljab -> ijab', I_mo5, t_amp)
    t_amp_new = (1/e_ijab)*(I_mo1 + (1/2)*np.einsum('abcd, ijcd -> ijab', I_movv, t_amp) + (1/2)*np.einsum('klij, klab -> ijab', I_mo3, t_amp) + \
    t_mixed - t_mixed.transpose((1, 0, 2, 3)) - t_mixed.transpose((0,1,3,2)) + t_mixed.transpose((1,0,3,2))   
    (1/4)*t_ccd1 + (1/2)(t_ccd2 - t_ccd2.tranpose(()) - t_ccd2.tranpose(()) + t_ccd2.tranpose(())) \
    - (1/2)*(t_ccd3 - t_ccd3.transpose(()) ) \
    - (1/2)*(t_ccd4 - t_ccd4.transpose(()) )    )              

    E_CCD = (1/4)*np.einsum('ijab, ijab ->', I_mo1, t_amp_new) 
