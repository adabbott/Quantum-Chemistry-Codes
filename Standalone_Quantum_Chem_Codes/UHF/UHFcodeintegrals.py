...
# Overlap integrals 
(S, T, Dx, Dy, Dz) = integrals.integrals()
...
for iteration in range(1, SCF_MAX_ITER+1):
    ...
    Da = np.einsum('pi,qi->pq', Ca, Ca)
    Db = np.einsum('pi,qi->pq', Cb, Cb)
nuc = psi4.core.nuclear_dipole(molecule).to_array()

mux = -np.einsum('mn,mn->', (Da + Db), Dx) + nuc[0]
muy = -np.einsum('mn,mn->', (Da + Db), Dy) + nuc[1]
muz = -np.einsum('mn,mn->', (Da + Db), Dz) + nuc[2]

mu = np.sqrt(mux**2 + muy**2 + muz**2)
print(mints.ao_dipole())
