...
def transformoei(hao, Q):
    return np.einsum('pq, qr -> pr', hao, Q)  
...
for i in range(OMP2_MAX_ITER):    
    h1 = transformoei(spinblockoei(H),Co) #spin blocks H and transforms
    ...
    tamp = I_mo[O,O,V,V] + (1/2)*(term2 - term2.transpose(0,1,3,2))                         \
                    - (1/2)*(term3 - term3.transpose(1,0,2,3))
    ...
    DM1cor[V,V] =  (1/2)*np.einsum('ijac,ijbc->ba',tamp,tamp)#these might be wrong
    DM1cor[O,O] = -(1/2)*np.einsum('jkab,ikab->ji',tamp,tamp)
    DM2cor[O,O,V,V] = tamp
    DM2cor[V,V,O,O] = tamp.T
    D1 = DM1cor + DM1ref
    D2_term2 = np.einsum('rp,sq->rspq',DM1cor,DM1ref)
    D2_term3 = np.einsum('rp,sq->rspq',DM1ref,DM1ref)
    D2 = DM2cor + (1/4)*(D2_term2 - D2_term2.transpose(1,0,2,3) - D2_term2.transpose(0,1,3,2) \
         + D2_term2.transpose(1,0,3,2)) + (1/2)*(D2_term3 - D2_term3.transpose(1,0,2,3))
    ...
    Cnew = Co.dot(U)
    h1new   = transformoei(h1, Cnew)
    I_monew = transformtei(I_mo, Cnew)
    EOMP2 = molecule.nuclear_repulsion_energy()             \
        + np.einsum('pq,qp->',h1new,D1)                 \
        + (1/4)*np.einsum('pqrs,rspq->',I_monew,D2)
    print(EOMP2)
