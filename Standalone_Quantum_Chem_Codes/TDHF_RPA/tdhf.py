...
B = np.zeros((nvirt*nocc,nvirt*nocc))
w = -1
strings = []
str2 = '->'
for i in range(nocc):
    for a in range(nocc, ntotal):
        ...
        for j in range(nocc):
            for b in range(nocc, ntotal):
                ...
                B[w,v] = I_mo[a,b,i,j]
#tdhf v1
TDHF = np.kron(np.array([[1,0],[0,-1]]), HCIS) + np.kron(np.array([[0,1],[-1,0]]), B)
tdhfval, tdhfvec = np.linalg.eig(TDHF)
print('TDHF Orbital energies, method 1:')
print(np.sort([x for x in tdhfval if x >= 0 ])) 
#tdhf v2
v2val, v2vec = np.linalg.eig((HCIS + B).dot(HCIS - B))
v2val = np.sqrt(v2val)
print('TDHF Orbital energies, method 2:')
print(np.sort(v2val))
