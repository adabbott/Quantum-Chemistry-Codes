...
for iteration in range(1, CEPA0_MAX_ITER+1):
    ...
    t_ccd1 = np.einsum('cdkl, klab, ijcd -> ijab', I_mo5, t_amp, t_amp)
    t_ccd2 = np.einsum('cdkl, ikac, ljdb -> ijab', I_mo5, t_amp, t_amp)
    t_ccd3 = np.einsum('cdkl, klca, ijdb -> ijab', I_mo5, t_amp, t_amp)
    t_ccd4 = np.einsum('cdkl, kicd, ljab -> ijab', I_mo5, t_amp, t_amp)
    ...
    (1/4)*t_ccd1 + (1/2)*(t_ccd2 - t_ccd2.transpose((1,0,2,3)) - t_ccd2.transpose((0,1,3,2)) + t_ccd2.transpose((1,0,3,2))) - \
    (1/2)*(t_ccd3 - t_ccd3.transpose((1,0,2,3))) - (1/2)*(t_ccd4 - t_ccd4.transpose((0,1,3,2))))
