...
for iteration in range(1, CEPA0_MAX_ITER+1):
    ...
    t_amp_new = (1/e_ijab)*(I_mo[O, O, V, V]  + (1/2)*np.einsum('ijkl, klab -> ijab', I_mo[O,O,O,O], t_amp) + np.einsum('cjkb, ikac -> ijab', I_mo[V,O,O,V], t_amp) - np.einsum('cikb, jkac -> ijab', I_mo[V,O,O,V], t_amp) - np.einsum('cjka, ikbc -> ijab', I_mo[V,O,O,V], t_amp) + np.einsum('cika, jkbc -> ijab', I_mo[V,O,O,V], t_amp)) 
