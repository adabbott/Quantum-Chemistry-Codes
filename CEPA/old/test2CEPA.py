...
#                E_MP2 += ((1/4)*I_mo[i, j, a, b]**2)/(orb_energies[i]+orb_energies[j]-orb_energies[a]-orb_energies[b])
 ...
 #for i in range(nocc):
 #    for j in range(nocc):
 #        #for a particular set of occupied indices 
...
for iteration in range(1, CEPA0_MAX_ITER+1):
    ...
    #t_amp_new = (1/e_ijab)*(I_mo[O, O, V, V]  + (1/2)*np.einsum('cdab, ijcd -> ijab', I_mo[V,V,V,V], t_amp) + (1/2)*np.einsum('ijkl, klab -> ijab', I_mo[O,O,O,O], t_amp) + np.einsum('cjkb, ikac -> ijab', I_mo[V,O,O,V], t_amp) - np.einsum('cikb, jkac -> jiab', I_mo[V,O,O,V], t_amp) - np.einsum('cjka, ikbc -> ijba', I_mo[V,O,O,V], t_amp) + np.einsum('cika, jkbc -> jiba', I_mo[V,O,O,V], t_amp)) 
    #t_amp_new = (1/e_ijab)*(I_mo[O, O, V, V]  + (1/2)*np.einsum('abcd, ijcd -> ijab', I_mo[V,V,V,V], t_amp) + (1/2)*np.einsum('klij, klab -> ijab', I_mo[O,O,O,O], t_amp)
    t_amp_new = np.einsum('kbcj, ikac -> ijab', I_mo[O,V,V,O], t_amp) -  np.einsum('kbci, jkac -> jiab', I_mo[O,V,V,O], t_amp) - np.einsum('kacj, ikbc -> ijba', I_mo[O,V,V,O], t_amp) + np.einsum('kaci, jkbc -> jiba', I_mo[O,V,V,O], t_amp)
    print(t_amp_new)
    print(np.linalg.norm(t_amp_new))
    #t4 = np.einsum('kbcj, ikac -> ijab', I_mo[O,V,V,O], t_amp)
    #t_amp_new = (1/e_ijab)*(I_mo[O, O, V, V]  + (1/2)*np.einsum('abcd, ijcd -> ijab', I_mo[V,V,V,V], t_amp) + (1/2)*np.einsum('klij, klab -> ijab', I_mo[O,O,O,O], t_amp) + t4 - t4.transpose((1,0,2,3)) - t4.transpose((0,1,3,2)) + t4.transpose((1,0,3,2)))
    #E_CEPA0 = (1/4)*np.einsum('ijab, ijab ->', I_mo[O,O,V,V], t_amp_new)
...
#        for a in range(nocc,ntotal):
#            for b in range(nocc,ntotal):  
#                E_CEPA0 += (1/4)*I_mo[a, b, i, j]*t_amp[i, j, a, b]
